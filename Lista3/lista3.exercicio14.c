...
void otimas(int idds[100], char avali[101][8]){
    int contO=0;
    for (int i=0; i<100; i++){
...
void difBR(int idds[100], char avali[101][8]){
    int dif, contB=0, contR=0;
    for (int i=0; i<100; i++){
    ...
    dif = contB-contR;  //Cálcula da diferença de porcentagem.
    printf("A diferença percentual entre as avaliações boas e regulares é de %d%%.\n", dif);
}

void mediaR(int idds[100], char avali[101][8]){
...
void porMaiP(int idds[100], char avali[101][8]){
    int maiorIDD=0, contP=0;
    for (int i=0; i<100; i++){
    ...
    printf("A maior idade que avaliou como 'péssimo' foi %d e a porcentagem das respostas foi de %d%%.\n", maiorIDD, contP);
}

void difOR(int idds[100], char avali[101][8]){
    int maiO=0, maiR=0;
    for (int i=0; i<100; i++){
        if(strcmp("otimo", avali[i])==0){   //Compara a avaliação com oque o usuário avaliou.
            if(idds[i]>maiO){
...
int main(void){
    int idds[101];  //Vetor que guarda as idades dos entrevistados.
    char avali[101][8]; //Vetor que guarda as avaliações.
